...
LimsManifestRepository extends JpaRepository<LIMSManifest, Integer> {
    ...
    @Query(value="SELECT id, manifest_record_id AS \"manifestRecordID\", sample_id AS \"sampleID\", test_id AS \"testID\",\n" +
            ...
            "priority_reason AS \"priorityReason\", unique_id AS \"uniqueId\" FROM lims_sample WHERE test_id  = ?1 AND sample_id  = ?2", nativeQuery = true)
    Optional<LIMSSampleProjection> findLIMSSampleBySampleIDAndTestID(Integer testId, String sampleId);
    ...
    @Query(value="SELECT id, manifest_record_id AS \"manifestRecordID\", sample_id AS \"sampleID\", test_id AS \"testID\",\n" +
            ...
            "surname AS \"surName\", sex, age, hospital_number AS \"hospitalNumber\", drug_regimen AS \"drugRegimen\",\n" +
            "sending_facility_id AS \"sendingFacilityID\", sending_facility_name AS \"sendingFacilityName\", priority, \n" +
            "priority_reason AS \"priorityReason\", unique_id AS \"uniqueId\" FROM lims_sample WHERE sample_id  = ?1", nativeQuery = true)
    Optional<LIMSSampleProjection> findLIMSSampleBySampleID(String sampleId);
